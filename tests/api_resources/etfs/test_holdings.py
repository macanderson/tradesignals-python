# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from tests.utils import assert_matches_type
from tradesignals import TradesignalsIo, AsyncTradesignalsIo
from tradesignals.types.etfs import HoldingHoldingsResponse

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestHoldings:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_holdings(self, client: TradesignalsIo) -> None:
        holding = client.etfs.holdings.holdings(
            "ticker",
        )
        assert_matches_type(HoldingHoldingsResponse, holding, path=["response"])

    @parametrize
    def test_method_holdings_with_all_params(self, client: TradesignalsIo) -> None:
        holding = client.etfs.holdings.holdings(
            "ticker",
        )
        assert_matches_type(HoldingHoldingsResponse, holding, path=["response"])

    @parametrize
    def test_raw_response_holdings(self, client: TradesignalsIo) -> None:
        response = client.etfs.holdings.with_raw_response.holdings(
            "ticker",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        holding = response.parse()
        assert_matches_type(HoldingHoldingsResponse, holding, path=["response"])

    @parametrize
    def test_streaming_response_holdings(self, client: TradesignalsIo) -> None:
        with client.etfs.holdings.with_streaming_response.holdings(
            "ticker",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            holding = response.parse()
            assert_matches_type(HoldingHoldingsResponse, holding, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_holdings(self, client: TradesignalsIo) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `ticker` but received ''"):
            client.etfs.holdings.with_raw_response.holdings(
                "",
            )


class TestAsyncHoldings:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_holdings(self, async_client: AsyncTradesignalsIo) -> None:
        holding = await async_client.etfs.holdings.holdings(
            "ticker",
        )
        assert_matches_type(HoldingHoldingsResponse, holding, path=["response"])

    @parametrize
    async def test_method_holdings_with_all_params(self, async_client: AsyncTradesignalsIo) -> None:
        holding = await async_client.etfs.holdings.holdings(
            "ticker",
        )
        assert_matches_type(HoldingHoldingsResponse, holding, path=["response"])

    @parametrize
    async def test_raw_response_holdings(self, async_client: AsyncTradesignalsIo) -> None:
        response = await async_client.etfs.holdings.with_raw_response.holdings(
            "ticker",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        holding = await response.parse()
        assert_matches_type(HoldingHoldingsResponse, holding, path=["response"])

    @parametrize
    async def test_streaming_response_holdings(self, async_client: AsyncTradesignalsIo) -> None:
        async with async_client.etfs.holdings.with_streaming_response.holdings(
            "ticker",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            holding = await response.parse()
            assert_matches_type(HoldingHoldingsResponse, holding, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_holdings(self, async_client: AsyncTradesignalsIo) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `ticker` but received ''"):
            await async_client.etfs.holdings.with_raw_response.holdings(
                "",
            )
