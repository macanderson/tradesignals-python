...
class TradesignalsIo(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        This automatically infers the `api_key` argument from the `TRADESIGNALS_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("TRADESIGNALS_API_KEY")
        if api_key is None:
            raise TradesignalsIoError(
                "The api_key client option must be set either by passing api_key to the client or by setting the TRADESIGNALS_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("TRADESIGNALS_IO_BASE_URL")
        if base_url is None:
